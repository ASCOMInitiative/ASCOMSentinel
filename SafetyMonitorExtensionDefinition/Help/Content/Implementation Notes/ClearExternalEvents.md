...
### Alpaca Clients
Alpaca clients should send an HTTP PUT request to the <code>ClearExternalEvents</code> endpoint with 
an application/x-www-form-urlencoded <code>SafetyEventIdList</code> body parameter 
containing a JSON array of string <code>SafetyEvent.Id</code> values.

### Alpaca Devices
Alpaca devices should expect an Alpaca PUT request to the <code>ClearExternalEvents</code> endpoint with 
an application/x-www-form-urlencoded <code>SafetyEventIdList</code> body parameter 
containing a JSON array of string <code>SafetyEvent.Id</code> values.

### COM Clients
COM clients must send an <code>ArrayList</code> of string <code>SafetyEvent.Id</code> values.

### COM Drivers
COM drivers should expect to receive an enumerable containing string <code>SafetyEvent.Id</code> values.

The variable receiving the client's request must be typed as <code>IEnumerable</code>
or <code>IEnumerable&lt;string&gt;</code> rather than as <code>ArrayList</code>, see note below.
